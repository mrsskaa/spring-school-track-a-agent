...
def _city_search_variants(city: str) -> list[str]:
    ...
    if lowered.endswith("е") and len(cleaned) > 1:
        variants.append(cleaned[:-1])
    if lowered.endswith("и") and len(cleaned) > 1:
        variants.append(cleaned[:-1])
        variants.append(cleaned[:-1] + "ь")
    if lowered.endswith("у") and len(cleaned) > 1:
        variants.append(cleaned[:-1] + "а")
    if lowered.endswith("ю") and len(cleaned) > 1:
        variants.append(cleaned[:-1] + "ь")
...
def get_weather(city: str) -> str:
    try:
        ...
        for city_variant in _city_search_variants(city):
            geo_data = _get_json(
                GEOCODING_URL,
                {"name": city_variant, "count": 1, "language": "ru", "format": "json"},
            )
        ...
        if not current:
            return json.dumps({"error": "Не удалось получить актуальную погоду."}, ensure_ascii=False)
        ...
        return json.dumps({"error": "Ошибка при запросе к погодному API. Попробуйте позже."}, ensure_ascii=False)
    except (KeyError, TypeError, ValueError):
        return json.dumps({"error": "Получены неожиданные данные от погодного API."}, ensure_ascii=False)
