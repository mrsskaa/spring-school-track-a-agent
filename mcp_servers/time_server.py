...
def _get_json(url: str, params: dict[str, object]) -> dict[str, object]:
    ...
    for _ in range(TIME_API_ATTEMPTS):
        try:
...
def get_local_time(timezone: str) -> str:
    try:
        ...
        if not local_time or not date:
            return json.dumps({"error": "Не удалось получить локальное время."}, ensure_ascii=False)
        response_payload = {"timezone": timezone, "local_time": f"{date} {local_time}"}
        return json.dumps(response_payload, ensure_ascii=False)
    except requests.RequestException:
        return json.dumps({"error": "Ошибка при запросе к API времени. Попробуйте позже."}, ensure_ascii=False)
    except (KeyError, TypeError, ValueError):
        return json.dumps({"error": "Получены неожиданные данные от API времени."}, ensure_ascii=False)
